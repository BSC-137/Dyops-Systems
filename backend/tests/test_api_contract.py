...
from backend.tests.conftest import wait_for_persisted_events as _wait_for_persisted_events
import sentinel
...
def test_telemetry_websocket_receives_event_result(client: TestClient) -> None:
    ...
    assert payload["peg_health"]["schema_version"]
    assert payload["peg_health"]["band"] in {
        "Healthy",
        "Watch",
        "Breach",
        "Audit",
